...
class DeviceSpec(_system: ActorSystem)
  ...
  "A Device actor" should "ignore invalid registration requests" in {
    val probe = TestProbe()
    val deviceActor = system.actorOf(Device.props("group", "device"))
  ...
  "A DeviceGroup actor" should "be able to collect temperatures from all active devices" in {
    val probe = TestProbe()
    val groupActor = system.actorOf(DeviceGroup.props("group"))
  ...
  "A DeviceGroupQuery actor" should "return temperature value for working devices" in {
  ...
  "A DeviceGroupQuery actor" should "return TemperatureNotAvailable for devices with no readings" in {
    val requester = TestProbe()
  ...
  "A DeviceGroupQuery actor" should "return DeviceNotAvailable if device stops before answering" in {
    val requester = TestProbe()
  ...
  "A DeviceGroupQuery actor" should "return temperature reading even if device stops after answering" in {
    val requester = TestProbe()
  ...
  "A DeviceGroupQuery actor" should "return DeviceTimedOut if device does not answer in time" in {
    val requester = TestProbe()
